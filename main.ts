...
export default class MyPlugin extends Plugin {
    ...
    async searchArxiv() {
        ...
        const query = `ti:"${selectedText.replace(/ /g, "+")}"`; // 선택된 텍스트를 제목 검색 쿼리로 사용
        try {
            const result = await this.fetchArxivData(query);
            const parsedResult = this.parseArxivResponse(result); // XML 파싱
            console.log(parsedResult);
            new Notice('검색 결과가 콘솔에 출력되었습니다.'); // 결과를 notice로 띄움
        } catch (e) {
    ...
    // Arxiv API에서 데이터를 가져오는 함수
    async fetchArxivData(query: string, maxRetries = 10) {
        const url = `https://export.arxiv.org/api/query?search_query=${query}&start=0&max_results=3`; // max_results를 3으로 변경
